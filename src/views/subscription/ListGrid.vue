  ...
  <TableCardHeader
    ...
    <div class="card">
        <div class="card-body table-div">
                ...
                <template v-slot:subscriptionTeacher="{ row: data }">
                  <info-item-table :data="data.subscriptionTeacher" type="hasOne" displayName="name" />
                </template>
                <template v-slot:subscriptionStudent="{ row: data }">
                  <info-item-table :data="data.subscriptionStudent" type="hasOne" displayName="username" />
                </template>
                <template v-slot:subscriptionLevel="{ row: data }">
                  <info-item-table :data="data.subscriptionLevel" type="hasOne" displayName="name" />
                </template>
                ...
                <template v-slot:actions="{ row: data, index }">
                    <div class="text-end">
                        <router-link
                          ...
                          :to="{ name: 'subscription-view', params: { id: data.id } }"
                          v-if="can('view', 'subscription')"
                          >
                        ...
                        <router-link
                          class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"
                          :to="{ name: 'subscription-edit',params: { id: data.id }}"
...
/* Providers */
import {tableHeader, filterFields, filterFormData} from '@/providers/subscription';
