...
import { formData, formFields, defaultFormFields } from "@/providers/center";
...
import {
  ...
  resetConstraints
    ...
    const centerItem = ref();
    const router = useRouter();
    const id = ref(router.currentRoute.value.params.id);

     const handleSubmit = async () => {
      const data = cloneDeep(formData.value)
      const res = await centerEntity.update(id.value.toString(), data)
      if (res) {
        Swal.fire({
          ...
          router.push(`/center-view/${res.id}`)
        })
      }
    }
    ...
    onMounted(async () => {
      centerItem.value = await centerEntity.fetchItemAsync(id.value)
