...
import { formData, formFields, defaultFormFields } from "@/providers/video";
  ...
  resetConstraints
    ...
    const id = ref(router.currentRoute.value.params.id);
     ...
     const handleSubmit = async () => {
      ...
      const res = await videoEntity.update(id.value.toString(), data)
      if (res) {
        Swal.fire({
    ...
    onMounted(async () => {
      videoItem.value = await videoEntity.fetchItemAsync(id.value)
