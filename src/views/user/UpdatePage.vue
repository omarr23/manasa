...
import { formData, formFields, defaultFormFields } from "@/providers/user";
  ...
  resetConstraints
    ...
    const id = ref(router.currentRoute.value.params.id);
     ...
     const handleSubmit = async () => {
      ...
      const res = await userEntity.update(id.value.toString(), data)
      if (res) {
        Swal.fire({
          text: `${filters.ConvertCamelToTitleName("user")} Created Successfully`,
          icon: 'success',
    ...
    onMounted(async () => {
      userItem.value = await userEntity.fetchItemAsync(id.value)
      setConstarinsOwnerAndBu(formFields, formData,'user_assign_ownership')
      getDefaultValues(formData, defaultFormFields,'user_assign_ownership')
    });
</script>
