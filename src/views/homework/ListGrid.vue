    ...
    <div class="card">
        <div class="card-body table-div">
                ...
                <template v-slot:description="{ row: data }">
                    <info-item-table :data="data.description" type="string"/>
                </template>
                ...
                <template v-slot:homeworkTeacher="{ row: data }">
                  <info-item-table :data="data.homeworkTeacher" type="hasOne" displayName="name" />
                </template>
                <template v-slot:homeworkVideo="{ row: data }">
                  <info-item-table :data="data.homeworkVideo" type="hasOne" displayName="title" />
                </template>
                <template v-slot:homeworkStudent="{ row: data }">
                  <info-item-table :data="data.homeworkStudent" type="hasOne" displayName="username" />
                </template>
                ...
                <template v-slot:actions="{ row: data, index }">
                    <div class="text-end">
                        <router-link
                          ...
                          :to="{ name: 'homework-view', params: { id: data.id } }"
                          v-if="can('view', 'homework')"
                          >
                        ...
                        <router-link
                          class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"
                          :to="{ name: 'homework-edit',params: { id: data.id }}"
                          v-if="can('update', 'homework')"
                          >
...
/* Providers */
import {tableHeader, filterFields, filterFormData} from '@/providers/homework';
