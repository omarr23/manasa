    ...
    <section>
        ...
        >
            <div
                ...
                id="kt_teacher_details_view"
            >
                <div class="card-header cursor-pointer">
                    <!--begin::Card title-->
                    <div class="card-title m-0">
                        <h3 class="fw-bold m-0">
                            {{filters.ConvertCamelToTitleName($t('teacher._title')) }} {{$t('teacher.details') }}
                        </h3>
                    </div>
                    <!--end::Card title-->
                    <div>
                        ...
                        >
                            <div class="menu-item px-5">
                                <router-link
                                    :to="{ name: 'teacher-edit', params: { id: viewData?.id } }"
                                    class="menu-link align-self-center"
                                    v-if="can('update', 'teacher')"
                                    >
                                    {{ $t('action.edit') }}
                                </router-link>
                            </div>
                        ...
                        <div v-if="viewData" class="row">
                            ...
                            <div class="col-12">
                                <div class="row">
                                    ...
                                    <div class="mb-3 col-lg-6">
                                        <info-item
                                            :label="$t('teacher.teacherCenter')"
                                            :data="viewData.teacherCenter"
                                            type="hasOne"
                                            displayName="name"
                                        />
                                    </div>
                                    ...
                                    <div class="mb-3 col-lg-6">
                                        <info-item
                                            :label="$t('teacher.teacheruser')"
                                            :data="viewData.teacheruser"
                                            type="hasOne"
                                            displayName="email"
                                        />
                                    </div>
            ...
            <ManyToManyRelationGrid
                ...
                :listData="levelManyToManyHandler.dataRef"
                :listModalData="levelManyToManyHandler.dataRefModal"
                :provider="levelProvider_"
                :relation="levelRelation"
                @handlePageChange="levelManyToManyHandler.handlePageChange"
                @handleSearch="levelManyToManyHandler.handleSearch"
                @handleSearchModal="levelManyToManyHandler.handleSearchModal"
                @handelExportReport="levelManyToManyHandler.handleExportReport"
                @handleOnPageSizeChange="levelManyToManyHandler.handleOnPageSizeChange"
                @handelOpenModal="levelManyToManyHandler.handelOpenModal"
                @handleAssign="levelManyToManyHandler.assignItem"
                @handleDetach="levelManyToManyHandler.detachItem"
                @handleFilter="levelManyToManyHandler.handleFilter"
                @handleFilterModal="levelManyToManyHandler.handleFilterModal"
                @handleResetFilter="levelManyToManyHandler.handleResetFilter"
                @handleResetFilterModal="levelManyToManyHandler.handleResetFilterModal"
            />
        </div>
    </section>
</template>
...
import type { teacher } from '@/providers/teacher'
import * as  teacherProvider_  from '@/providers/teacher'
import {relations} from '@/providers/teacher'
...
const viewData = ref<teacher|null>();
...
const levelRelation=teacherProvider_.relations.levelRelation
const levelManyToManyHandler = useManyToManyHandler(useLevelStore(),{ filter: `${ levelRelation.relationName}.some.${ levelRelation.relationKeyFilter} eq ${id.value}  ` })
