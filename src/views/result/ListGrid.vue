    ...
    <div class="card">
        <div class="card-body table-div">
                ...
                <template v-slot:total="{ row: data }">
                    <info-item-table :data="data.total" type="int"/>
                </template>
                ...
                <template v-slot:resultTeacher="{ row: data }">
                  <info-item-table :data="data.resultTeacher" type="hasOne" displayName="name" />
                </template>
                <template v-slot:resultStudent="{ row: data }">
                  <info-item-table :data="data.resultStudent" type="hasOne" displayName="username" />
                </template>
                ...
                <template v-slot:actions="{ row: data, index }">
                    <div class="text-end">
                        <router-link
                          ...
                          :to="{ name: 'result-view', params: { id: data.id } }"
                          v-if="can('view', 'result')"
                          >
                        ...
                        <router-link
                          class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"
                          :to="{ name: 'result-edit',params: { id: data.id }}"
...
/* Providers */
import {tableHeader, filterFields, filterFormData} from '@/providers/result';
