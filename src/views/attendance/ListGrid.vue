  ...
  <TableCardHeader
    ...
    <div class="card">
        <div class="card-body table-div">
                ...
                >
                <template v-slot:attended="{ row: data }">
                    <info-item-table :data="data.attended" type="boolean"/>
                </template>
                ...
                <template v-slot:attendanceTeacher="{ row: data }">
                  <info-item-table :data="data.attendanceTeacher" type="hasOne" displayName="name" />
                </template>
                <template v-slot:attendanceLevel="{ row: data }">
                  <info-item-table :data="data.attendanceLevel" type="hasOne" displayName="name" />
                </template>
                <template v-slot:attendanceStudent="{ row: data }">
                  <info-item-table :data="data.attendanceStudent" type="hasOne" displayName="username" />
                </template>
                <template v-slot:attendanceVideo="{ row: data }">
                  <info-item-table :data="data.attendanceVideo" type="hasOne" displayName="title" />
                </template>
                ...
                <template v-slot:actions="{ row: data, index }">
                    <div class="text-end">
                        <router-link
                          ...
                          :to="{ name: 'attendance-view', params: { id: data.id } }"
                        ...
                        <router-link
                          class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"
                          :to="{ name: 'attendance-edit',params: { id: data.id }}"
...
/* Providers */
import {tableHeader, filterFields, filterFormData} from '@/providers/attendance';
