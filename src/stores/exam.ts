...
import { formData, type exam, withArray } from '@/providers/exam'
...
// Use this for shared components, I would recommend a map object
  ...
  function fetchListRef(
    query?: FilterQuery,
    reference?: Ref<PaginationResponse<exam>>
  ): Ref<PaginationResponse<exam>> {
    ...
    const listData = reference ?? ref<PaginationResponse<exam>>(DefaultResponse)
  ...
  async function fetchListAsync(
    ...
    const { data } = await ApiService.query('exam', {
  ...
  function fetchItemRef(
    ...
    // I am not sure about the behavior of ref function from vue, so I am assuming it copies into new reference
    // if not, use the set defaults as follows: setDefaults({}, defaultResponse)
    const itemData = reference ?? ref<exam | null>(null)
  ...
  async function fetchItemAsync(
    id: String,
    query?: FilterQuery,
    reference?: Ref<exam | null>
  ): Promise<exam> {
    
    // TODO: dont try catch here, because logic may differ from page to page
    const { data } = await ApiService.query(`exam/${id}`, {
      params: {...query, with:  withArray.join()},
    })
    if (reference) {
      reference.value = data
  ...
  async function exportReport() {
    try {
      const { data: responseData } = await ApiService.query("exam/export", {
        params: {},
      });
...
async function exportCSV() {
  try {
    const { data: responseData } = await ApiService.query("exam/export/csv", {
      params: {},
    });
...
async function importCSV(payload: Object) {
  const { data } = await ApiService.post('exam/import', payload)
  return data
}

async function create(payload: exam): Promise<exam> {
    const { data } = await ApiService.post('exam', payload)
    return data
  }

  async function update(id: string | number, payload: typeof formData.value): Promise<exam> {
    const { data } = await ApiService.update(`exam/${id}`, payload)
  ...
  async function deleteItem(id: number | string): Promise<exam> {
    const { data } = await ApiService.delete(`exam/${id}`)
    return data
  }
