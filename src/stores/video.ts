...
import { formData, type video, withArray } from '@/providers/video'
...
// Use this for shared components, I would recommend a map object
  ...
  function fetchListRef(
    query?: FilterQuery,
    reference?: Ref<PaginationResponse<video>>
  ): Ref<PaginationResponse<video>> {
    ...
    const listData = reference ?? ref<PaginationResponse<video>>(DefaultResponse)
  ...
  async function fetchListAsync(
    ...
    const { data } = await ApiService.query('video', {
  ...
  function fetchItemRef(
    ...
    // I am not sure about the behavior of ref function from vue, so I am assuming it copies into new reference
    // if not, use the set defaults as follows: setDefaults({}, defaultResponse)
    const itemData = reference ?? ref<video | null>(null)
  ...
  async function fetchItemAsync(
    id: String,
    query?: FilterQuery,
    reference?: Ref<video | null>
  ): Promise<video> {
    
    // TODO: dont try catch here, because logic may differ from page to page
    const { data } = await ApiService.query(`video/${id}`, {
      params: {...query, with:  withArray.join()},
    })
    if (reference) {
      reference.value = data
  ...
  async function exportReport() {
    try {
      const { data: responseData } = await ApiService.query("video/export", {
        params: {},
      });
...
async function exportCSV() {
  try {
    const { data: responseData } = await ApiService.query("video/export/csv", {
      params: {},
    });
...
async function importCSV(payload: Object) {
  const { data } = await ApiService.post('video/import', payload)
  return data
}

async function create(payload: video): Promise<video> {
    const { data } = await ApiService.post('video', payload)
    return data
  }

  async function update(id: string | number, payload: typeof formData.value): Promise<video> {
    const { data } = await ApiService.update(`video/${id}`, payload)
  ...
  async function deleteItem(id: number | string): Promise<video> {
    const { data } = await ApiService.delete(`video/${id}`)
    return data
  }
