...
import { formData, type student, withArray } from '@/providers/student'
...
// Use this for shared components, I would recommend a map object
  ...
  function fetchListRef(
    query?: FilterQuery,
    reference?: Ref<PaginationResponse<student>>
  ): Ref<PaginationResponse<student>> {
    ...
    const listData = reference ?? ref<PaginationResponse<student>>(DefaultResponse)
  ...
  async function fetchListAsync(
    ...
    const { data } = await ApiService.query('student', {
  ...
  function fetchItemRef(
    id: String,
    query?: FilterQuery,
    reference?: Ref<student>
  ): Ref<student | null> {
    // // first get or create a new reference
    // I am not sure about the behavior of ref function from vue, so I am assuming it copies into new reference
    // if not, use the set defaults as follows: setDefaults({}, defaultResponse)
    const itemData = reference ?? ref<student | null>(null)
  ...
  async function fetchItemAsync(
    id: String,
    query?: FilterQuery,
    reference?: Ref<student | null>
  ): Promise<student> {
    
    // TODO: dont try catch here, because logic may differ from page to page
    const { data } = await ApiService.query(`student/${id}`, {
      params: {...query, with:  withArray.join()},
    })
    if (reference) {
      reference.value = data
  ...
  async function exportReport() {
    try {
      const { data: responseData } = await ApiService.query("student/export", {
        params: {},
      });
...
async function exportCSV() {
  try {
    const { data: responseData } = await ApiService.query("student/export/csv", {
      params: {},
    });
...
async function importCSV(payload: Object) {
  const { data } = await ApiService.post('student/import', payload)
  return data
}

async function create(payload: student): Promise<student> {
    const { data } = await ApiService.post('student', payload)
    return data
  }

  async function update(id: string | number, payload: typeof formData.value): Promise<student> {
    const { data } = await ApiService.update(`student/${id}`, payload)
  ...
  async function deleteItem(id: number | string): Promise<student> {
    const { data } = await ApiService.delete(`student/${id}`)
    return data
  }
