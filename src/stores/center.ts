...
import { formData, type center, withArray } from '@/providers/center'
...
// Use this for shared components, I would recommend a map object
  ...
  function fetchListRef(
    query?: FilterQuery,
    reference?: Ref<PaginationResponse<center>>
  ): Ref<PaginationResponse<center>> {
    ...
    const listData = reference ?? ref<PaginationResponse<center>>(DefaultResponse)
  ...
  async function fetchListAsync(
    query?: FilterQuery,
    reference?: Ref<PaginationResponse<center>>
  ): Promise<PaginationResponse<center>> {
    ...
    const { data } = await ApiService.query('center', {
  ...
  function fetchItemRef(
    id: String,
    query?: FilterQuery,
    reference?: Ref<center>
  ): Ref<center | null> {
    // // first get or create a new reference
    // I am not sure about the behavior of ref function from vue, so I am assuming it copies into new reference
    // if not, use the set defaults as follows: setDefaults({}, defaultResponse)
    const itemData = reference ?? ref<center | null>(null)
  ...
  async function fetchItemAsync(
    id: String,
    query?: FilterQuery,
    reference?: Ref<center | null>
  ): Promise<center> {
    
    // TODO: dont try catch here, because logic may differ from page to page
    const { data } = await ApiService.query(`center/${id}`, {
      params: {...query, with:  withArray.join()},
    })
    if (reference) {
      reference.value = data
  ...
  async function exportReport() {
    try {
      const { data: responseData } = await ApiService.query("center/export", {
        params: {},
      });
...
async function exportCSV() {
  try {
    const { data: responseData } = await ApiService.query("center/export/csv", {
      params: {},
    });
...
async function importCSV(payload: Object) {
  const { data } = await ApiService.post('center/import', payload)
  return data
}

async function create(payload: center): Promise<center> {
    const { data } = await ApiService.post('center', payload)
    return data
  }

  async function update(id: string | number, payload: typeof formData.value): Promise<center> {
    const { data } = await ApiService.update(`center/${id}`, payload)
  ...
  async function deleteItem(id: number | string): Promise<center> {
    const { data } = await ApiService.delete(`center/${id}`)
    return data
  }
