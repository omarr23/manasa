...
import { formData, type submissions, withArray } from '@/providers/submissions'
...
// Use this for shared components, I would recommend a map object
  ...
  function fetchListRef(
    query?: FilterQuery,
    reference?: Ref<PaginationResponse<submissions>>
  ): Ref<PaginationResponse<submissions>> {
    ...
    const listData = reference ?? ref<PaginationResponse<submissions>>(DefaultResponse)
  ...
  async function fetchListAsync(
    query?: FilterQuery,
    reference?: Ref<PaginationResponse<submissions>>
  ): Promise<PaginationResponse<submissions>> {
    ...
    const { data } = await ApiService.query('submissions', {
  ...
  function fetchItemRef(
    id: String,
    query?: FilterQuery,
    reference?: Ref<submissions>
  ): Ref<submissions | null> {
    // // first get or create a new reference
    // I am not sure about the behavior of ref function from vue, so I am assuming it copies into new reference
    // if not, use the set defaults as follows: setDefaults({}, defaultResponse)
    const itemData = reference ?? ref<submissions | null>(null)
  ...
  async function fetchItemAsync(
    id: String,
    query?: FilterQuery,
    reference?: Ref<submissions | null>
  ): Promise<submissions> {
    
    // TODO: dont try catch here, because logic may differ from page to page
    const { data } = await ApiService.query(`submissions/${id}`, {
      params: {...query, with:  withArray.join()},
    })
    if (reference) {
      reference.value = data
  ...
  async function exportReport() {
    try {
      const { data: responseData } = await ApiService.query("submissions/export", {
        params: {},
      });
...
async function exportCSV() {
  try {
    const { data: responseData } = await ApiService.query("submissions/export/csv", {
      params: {},
    });
...
async function importCSV(payload: Object) {
  const { data } = await ApiService.post('submissions/import', payload)
  return data
}

async function create(payload: submissions): Promise<submissions> {
    const { data } = await ApiService.post('submissions', payload)
    return data
  }

  async function update(id: string | number, payload: typeof formData.value): Promise<submissions> {
    const { data } = await ApiService.update(`submissions/${id}`, payload)
  ...
  async function deleteItem(id: number | string): Promise<submissions> {
    const { data } = await ApiService.delete(`submissions/${id}`)
    return data
  }
