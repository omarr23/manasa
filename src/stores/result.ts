...
import { formData, type result, withArray } from '@/providers/result'
...
// Use this for shared components, I would recommend a map object
  ...
  function fetchListRef(
    query?: FilterQuery,
    reference?: Ref<PaginationResponse<result>>
  ): Ref<PaginationResponse<result>> {
    ...
    const listData = reference ?? ref<PaginationResponse<result>>(DefaultResponse)
  ...
  async function fetchListAsync(
    ...
    const { data } = await ApiService.query('result', {
  ...
  function fetchItemRef(
    ...
    // I am not sure about the behavior of ref function from vue, so I am assuming it copies into new reference
    // if not, use the set defaults as follows: setDefaults({}, defaultResponse)
    const itemData = reference ?? ref<result | null>(null)
  ...
  async function fetchItemAsync(
    id: String,
    query?: FilterQuery,
    reference?: Ref<result | null>
  ): Promise<result> {
    
    // TODO: dont try catch here, because logic may differ from page to page
    const { data } = await ApiService.query(`result/${id}`, {
      params: {...query, with:  withArray.join()},
    })
    if (reference) {
      reference.value = data
  ...
  async function exportReport() {
    try {
      const { data: responseData } = await ApiService.query("result/export", {
        params: {},
      });
...
async function exportCSV() {
  try {
    const { data: responseData } = await ApiService.query("result/export/csv", {
      params: {},
    });
...
async function importCSV(payload: Object) {
  const { data } = await ApiService.post('result/import', payload)
  return data
}

async function create(payload: result): Promise<result> {
    const { data } = await ApiService.post('result', payload)
    return data
  }

  async function update(id: string | number, payload: typeof formData.value): Promise<result> {
    const { data } = await ApiService.update(`result/${id}`, payload)
  ...
  async function deleteItem(id: number | string): Promise<result> {
    const { data } = await ApiService.delete(`result/${id}`)
    return data
  }
